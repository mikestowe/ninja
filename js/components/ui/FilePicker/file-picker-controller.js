...
var FilePickerController = exports.FilePickerController = Montage.create(require("montage/ui/component").Component, {
    ...
    showFilePicker:{
        ...
        value:function(settings){
            ...
            var driveData = fileSystem.getDirectoryContents({uri:"", recursive:false, returnType:"all"});
    ...
    prepareContentList:{
        ...
        value:function(folderUri, aModel, fromCache, checkStaleness){
            ...
            if(((typeof fromCache !== "undefined") && (fromCache === false))
                ...
                try{
                    var iodata = fileSystem.getDirectoryContents({uri:folderUri, recursive:false, returnType:"all"});
    ...
    checkIfStale: {
        ...
        value: function(folderUri){
            ...
            if((new Date()).getTime() > (this._directoryContentCache[folderUri].queriedTimeStamp + this.cacheStaleTime)){
                try{
                    var ifModifiedResponse = fileSystem.isDirectoryModified({uri:folderUri, recursive:false, returnType:"all"}, this._directoryContentCache[folderUri].queriedTimeStamp);
